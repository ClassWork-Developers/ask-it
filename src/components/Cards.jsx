...
export default function Card({ form }) {
	...
	return (
		<>
			<ul className={'bg-'+theme+'-300 w-1/2 mx-auto rounded-lg text-xl m-3 cursor-pointer'}>
				<li>
					<h3 className="flex p-5 items-center justify-between" onClick={() => Show()}>
				...
				{select.map((pre, preI) => (
					<li
						...
						onClick={() => {
							aggPregunta([
								...preguntas,
								{ id: pre._id, pregunta: pre.pregunta, list: true },
							]);
							form(preguntas);
						}}
					>
			...
			{preguntas.map((pre, preIndex) => (
				<div className="w-1/2 mx-auto" key={preIndex}>
					<div className="flex justify-end">
						...
						<button
							...
							onClick={() => {
								aggPregunta([
									...preguntas,
									{ id: nextId, pregunta: pre.pregunta, list: false },
								]);
								aggnextId(nextId + 1);
								form(preguntas);
							}}
						...
						{/* Eliminar pregunta */}
						<button className="mt-1 p-1 px-2" onClick={() => eliminarPregunta(pre.id)}>
							<TrashIcon className="block h-6 w-6" aria-hidden="true" />
							{/* {pregunta.id} */}
						</button>
					</div>
					{/* card */}
					<div className={'bg-'+theme+'-100 dark:bg-'+theme+'-600 p-5 my-3 mt-1 rounded-md'}>
						<div className="flex">
