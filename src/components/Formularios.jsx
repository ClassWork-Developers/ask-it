...
export default function Formularios({ busqueda }) {
  ...
  const { mutate: Formularios } = useMutation(
    (data) =>
      axios.get(
        busqueda == "general"
          ? "http://localhost:3000/FullData"
          : `http://localhost:3000/DataFilter/${user.id}`
      ),
    {
      onSuccess: (response) => {
  ...
  if (forms.length) {
    ...
    <div className="bg-gray-100 dark:bg-gray-700">
      <div className="mx-auto p-4 max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {/* <Formulario form={} /> */}
        <p className="mt-6 space-y-12">Actualmente no hay formularios</p>
      </div>
    </div>;
  }
}
